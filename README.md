...
| **📆 Monthly** | Monthly stability overview (Production only) with one slide per section — Coupons, Loyalty, New Promotion V3 (Garuda UI), Tesseract (UI), and Badges (UI + Incentives Node) — each with a trend chart and a per-cluster pass-rate table. |
